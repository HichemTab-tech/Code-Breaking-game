...
<head>
	...
	<meta property="image" content="https://hichemtech.works/works/Code-Breaking-game/source/big-picture.png">
	<meta property="image:secure_url" content="https://hichemtech.works/works/Code-Breaking-game/source/big-picture.png" />
	<meta property="image:alt" content="Code Breaking game (Logo)" />

	<!-- Open Graph / Facebook -->
	<meta property="og:type" content="website">
	<meta property="og:url" content="https://hichemtech.works/works/Code-Breaking-game">
	...
	<meta property="og:image" content="https://hichemtech.works/works/Code-Breaking-game/source/big-picture.png">
	<meta property="og:image:secure_url" content="https://hichemtech.works/works/Code-Breaking-game/source/big-picture.png" />
	<meta property="og:image:alt" content="Code Breaking game (Logo)" />
	<meta property="fb:app_id" content="717203739588036"/>

	<!-- Twitter -->
	<meta property="twitter:card" content="summary_large_image">
	<meta property="twitter:url" content="https://hichemtech.works/works/Code-Breaking-game">
	...
	<meta property="twitter:image" content="https://hichemtech.works/works/Code-Breaking-game/source/big-picture.png">
	<meta property="twitter:image:secure_url" content="https://hichemtech.works/works/Code-Breaking-game/source/big-picture.png" />
...
<body onload="document.getElementById('wait').style.display='none';">
	...
	<div id="menu">
		...
		<div class="setting">
			<h2>Difficulty : </h2>
			<div class="difficulty difficulty_active">Easy</div><div class="difficulty">Meduim</div><div class="difficulty">Hard</div><div class="difficulty">Extreme</div>
		</div>
		<div class="Copyright">Ⓒ Copyright 2022 HichemTech, All rights reserved</div>
	</div>
	<div id="all0">
		<div id="all">
	...
	<div id="menu2">
		...
		<script>
			...
			var WARNING_THRESHOLD = TIME_LIMIT * 2 / 3;
			var ALERT_THRESHOLD = TIME_LIMIT / 3;
	...
	<div id="myModal2" class="modal2">
		<div class="modal2-content">
			...
			<div class="groupe_button2">
				...
				<div class="button_final2" onclick="confirme()">
					yes
				</div>
			</div>
		</div>
	...
	<script type="text/javascript">
		...
		var rows0 = document.getElementsByClassName('row_num');
		var rows1 = document.getElementsByClassName('row_rep');
		var rows2 = document.getElementsByClassName('row');
		var column = document.getElementsByClassName('column');
		var index_of_row_selected = rows2.length - 1;
		var index_of_column_selected = column.length - 1;
		// Get the modal
		...
		var var_code_true;
		var code_true;
		var numbers_probalities;
		var var_numbers_probalities;
		//
		var num_true_true = 0;
		var num_exist = 0;
		var num_true_false = 0;
		var num_false_false = 0;

		/* Saving game */
		var truecode_stocked = [];
		var big_array_coins = [];
		var big_array_rep = [];
		/*/// Saving game */
		//
		...
		function to_next_row() {
			...
			rowActive.addClass("opactity_moy");
			...
			document.getElementById('number_try').innerHTML = Number(document.getElementById('number_try').innerHTML) + 1;
		}

		document.onkeydown = function (event) {
			if (event.keyCode === 37 && can_play) {
				to_next_column();
			}
			else if (event.keyCode === 39 && can_play){
				to_previous_column();
			}
			else if ((event.keyCode <= 105 && event.keyCode >= 96) && can_play){
				if ((difficulty === "Meduim" || difficulty === "Extreme") || ((difficulty === "Easy" || difficulty === "Hard") && (event.keyCode - 96) !== 9 && (event.keyCode - 96) !== 8)) {
					var n = document.getElementsByClassName('number');
					setCoinNumber(column[index_of_column_selected], n[event.keyCode - 96].innerHTML, "other");
					to_next_column();
				}
			}
			else if (event.keyCode === 13 && can_play){
			...
			else if (event.keyCode === 27 && modal2.style.display === "block"){
				cancel();
			}
			else if (event.keyCode === 27 && modal.style.display === "block"){
		...
		function setCoinNumber(element, innerHtml, type_) {
			if (type_ === "img") {
				element.innerHTML = "<img src=" + innerHtml + "'assets/source/.png' class='coin' alt='coin'>";
			}
			else if (type_ === "text"){
		...
		function setCode(difficulty_) {
			if (localStorage.getItem("arr_truecode") === "" || localStorage.getItem("arr_truecode") === null) {
				...
				for (let i = document.getElementsByClassName('column_final').length - 1; i >= 0; i--) {
					document.getElementsByClassName('column_final')[i].innerHTML = "<img src=" + code_true[6 - i] + "'assets/source/.png' class='coin' alt='coin'>";
				}
		...
		function check() {
			if (can_play) {
				...
				setReponse(num_true_true, num_true_false, num_false_false);
		...
		function side_bar() {
			var side_bar = document.getElementById('menu_mob');
		...
		function setReponse(true_true, true_false, false_false) {
        ...
        $(document).ready(function () {
            ...
            if (localStorage.getItem("arr_coins") !== "" && localStorage.getItem("arr_coins") !== null) {
                ...
                for (let i = 0; i < JSON.parse(localStorage.getItem("arr_coins")).length; i++) {
                    big_array_coins[i] = [];
                    big_array_rep[i] = [];
                    for (var x = 0; x < (JSON.parse(localStorage.getItem("arr_coins")))[i].length; x++) {
                        big_array_coins[i][x] = JSON.parse(localStorage.getItem("arr_coins"))[i][x];
                    }
                    for (var y = 0; y < (JSON.parse(localStorage.getItem("arr_rep")))[i].length; y++) {
                        big_array_rep[i][y] = JSON.parse(localStorage.getItem("arr_rep"))[i][y];
                    }
                }
                for (let i = rows2.length - 1; i > index_of_row_selected; i--) {
                    rows2[rows2.length - 1 + index_of_row_selected - i + 1].getElementsByClassName('column')[0].innerHTML = "<img src="+big_array_coins[i - index_of_row_selected - 1][0]+"'assets/source/.png' class='coin' alt='coin'>";
                    rows2[rows2.length - 1 + index_of_row_selected - i + 1].getElementsByClassName('column')[1].innerHTML = "<img src="+big_array_coins[i - index_of_row_selected - 1][1]+"'assets/source/.png' class='coin' alt='coin'>";
                    rows2[rows2.length - 1 + index_of_row_selected - i + 1].getElementsByClassName('column')[2].innerHTML = "<img src="+big_array_coins[i - index_of_row_selected - 1][2]+"'assets/source/.png' class='coin' alt='coin'>";
                    rows2[rows2.length - 1 + index_of_row_selected - i + 1].getElementsByClassName('column')[3].innerHTML = "<img src="+big_array_coins[i - index_of_row_selected - 1][3]+"'assets/source/.png' class='coin' alt='coin'>";
                    rows2[rows2.length - 1 + index_of_row_selected - i + 1].getElementsByClassName('column')[4].innerHTML = "<img src="+big_array_coins[i - index_of_row_selected - 1][4]+"'assets/source/.png' class='coin' alt='coin'>";
                    rows2[rows2.length - 1 + index_of_row_selected - i + 1].getElementsByClassName('column')[5].innerHTML = "<img src="+big_array_coins[i - index_of_row_selected - 1][5]+"'assets/source/.png' class='coin' alt='coin'>";
                    rows2[rows2.length - 1 + index_of_row_selected - i + 1].getElementsByClassName('column')[6].innerHTML = "<img src="+big_array_coins[i - index_of_row_selected - 1][6]+"'assets/source/.png' class='coin' alt='coin'>";
